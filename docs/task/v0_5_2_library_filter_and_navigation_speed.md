...
- 메인 페이지도 hero 이후 섹션이 한 번에 막히지 않도록 로드 단위를 분리한다.
- 필터 검색 입력과 버튼의 정렬/높이를 정리해 UI 완성도를 맞춘다.
...
- 메인 페이지도 선반/라이브러리 섹션이 함께 묶여 늦게 보일 수 있다.
- 검색 아이콘 정렬과 필터 버튼 높이가 입력 필드와 맞지 않아 UI가 어색하다.
...
- 메인 페이지는 hero, curation shelves, library browser를 분리해 느린 구간이 전체 첫 렌더를 막지 않게 한다.
- 필터 입력과 버튼은 동일한 높이와 정렬 기준을 사용하도록 맞춘다.
- 확인된 병목만 최소 변경으로 줄인다.
...
- 메인 페이지 진입 시 hero 이후 섹션이 `Suspense` 기준으로 순차 노출된다.
- 필터 입력의 검색 아이콘과 `Filters` 버튼 높이가 입력 필드와 시각적으로 일치한다.
